...
args_path = os.path.join(args.outf, 'run_args.txt')
with open(args_path, 'w') as f:
    f.write(str(args))
    f.flush()
    print('Run arguments (saved to %s):' % args_path)
    print(args)
...
def train(epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        KL_fake_output = F.softmax(model(fake), dim=1)
        ...
        if batch_idx % args.log_interval == 0:
            print('Classification Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}, KL fake Loss: {:.6f}'.format(
                epoch, batch_idx * len(data), len(train_loader.dataset),
                       100. * batch_idx / len(train_loader), loss.item(), KL_loss_fake.item()))
